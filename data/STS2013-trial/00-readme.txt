...
The trial dataset contains the following:
  ...
  sts2012-paper.pdf		  paper describing the STS 2012 task
  sts2012-train.tgz		  train data from STS 2012
  sts2012-test.tgz                test data from STS 2012
...
The trial dataset comprises the 2012 dataset, which can be used to
develop and train systems.

In addition, we include sample data for the core test datasets, coming
from the following:
...
sentences will occur in each of the test datasets. Note that the OnWN
and FNWN test sets will be smaller, around 50% of the other two
datasets (headlines and SMT), so we will have approximately the same
number of pairs on each of the three kinds of pairs.
...
- STS.input.FNWN.txt: The sentences are sense definitions from WordNet
  and FrameNet. 5 pairs of definitions. Note that some FrameNet
  definitions involve more than one sentence.
...
- STS.input.SMT.txt: This SMT dataset comes from DARPA GALE HTER and
  ...
  described in (Dreyer and Marcu, NAACL 2012). 5 pairs of sentences
  where the reference comes from post edited translations.

The sentence pairs have been manually tagged with a number from 0 to
5, as defined below (cf. Gold Standard section). In this trial
dataset, this is just a dummy number which you can ignore.
...
- the data from Ontonotes / Wordnet mapping (or framenet / wordnet)
